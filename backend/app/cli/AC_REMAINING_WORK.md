...
6. ~~Guided delegation creation form~~ — done (Change Contract editing still isn't): `backend/app/tui/delegation_screen.py`'s `DelegationScreen`, pushed via `d`. Lists delegations, creates one via a real form (grantee UUID, comma-separated scopes, TTL) requiring `--grantor-id`, and revokes the selected row — all through the existing `ApiClient` methods. Change Contract editing (the other half of item 6) is still not implemented anywhere, CLI or TUI.
