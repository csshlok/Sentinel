...
2. ~~Lifecycle stepper widget~~ — done: `lifecycle_stepper()` in `detail_screen.py`, shown at the top of the detail view. Renders the main happy path (`DRAFT -> ... -> STABLE`) with the current state bracketed/bolded; off-path states (`PAUSED`/`BLOCKED`/`FAILED`/`CANCELLED`/`RECOVERY_*`) render on their own line instead of being forced onto a path they aren't on.
3. ~~Git/dependency evidence tables~~ and 4. ~~Assurance progress/results panel~~ — done, combined into one screen: `backend/app/tui/evidence_screen.py`'s `EvidenceScreen`, pushed via `g`. Shows Git checkpoints, environment passport/drift counts, dependency changes (+ unsupported-ecosystem warnings), the latest assurance plan's checks/coverage gaps, and the `[KB]`-owned lifecycle-gating facts (`required_assurance_passed`, `assurance_fresh`, etc., each with its `reasons`). Each of the five underlying `GET` routes is fetched independently and missing data renders an honest "not captured yet" line rather than omitting the section or guessing. `format_evidence()` is pure and tested (4 tests).
...
6. ~~Guided delegation creation form~~ — done: `backend/app/tui/delegation_screen.py`'s `DelegationScreen`, pushed via `d`. Lists delegations, creates one via a real form (grantee UUID, comma-separated scopes, TTL) requiring `--grantor-id`, and revokes the selected row. **Change Contract editing is done in both the CLI** (`change contract-update <id> <expected-revision> [--allowed-path ...] [--forbidden-path ...] [--required-check ...] [--authority-scope ...] [--max-risk ...]`, `ApiClient.update_change_contract`) **and the TUI** (`backend/app/tui/contract_screen.py`'s `ContractScreen`, bound to `c`, a guided form over the same client method; the dashboard now tracks each row's `revision` for optimistic-concurrency safety).
...
9. ~~Interaction-test coverage with Textual's `Pilot`/`run_test()`~~ — done, and a prior version of this document was **wrong**: `anyio`'s pytest plugin, already installed transitively via FastAPI (visible as `plugins: anyio-4.15.1` in every test run all session), runs `@pytest.mark.anyio` async tests with an `anyio_backend` fixture just fine — no `pytest-asyncio` or any new dependency was ever needed. `backend/tests/tui/test_pilot_interaction.py` (6 tests) covers: mounting at 80x24 and 120x30, the `r` refresh binding, `i` pushing/`escape` popping the detail screen, `v` pushing the recovery screen, and `q` quitting. **Writing this test found and fixed a real bug**: the dashboard's detail-view binding was on `enter`, which `DataTable` already binds internally for row selection, so the app-level binding never fired; rebound to `i`.
10. ~~Small-terminal/resize and `NO_COLOR` behavior~~ — done, and this was **also wrong to call blocked**: Textual sets `App.no_color` from the `NO_COLOR` env var automatically and applies a `Monochrome`/`NoColor` filter to all output (`textual/app.py`) - no app code was needed for the mechanism. `backend/tests/tui/test_no_color.py` (11 tests) verifies the flag/filter actually engage, that the dashboard still mounts correctly under it at both 80x24 and 120x30, and - the real durable guarantee - that every status label (`state_label`, `outcome_status_label`) carries a text symbol that survives stripping the colour markup, so nothing becomes unreadable once colour is gone. The only thing left that no automated test can replace is a human's subjective visual check at a real physical terminal.
